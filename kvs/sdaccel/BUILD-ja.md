...
- Scalars
  - 数は1のまま．`Argument name` を `data_num` に変更して，Next．
- Global Memory
  - そのままNext
...
- 追加
  ...
  - `/home/centos/axonerve_util/kvs/sdaccel/src/xilinx-ip/aws-f1-vu9p` の 下の xciファイル．これはプロジェクトにコピーする
...
- `host_example.cpp`を削除．`axonerve_kvs.cpp`，`axonerve_kvs.hpp`，`host.cpp`，`xcl2.cpp`，`xcl2.hpp`を追加
  ...
  - `axonerve_kvs.cpp`，`axonerve_kvs.hpp`，`host.cpp`，`xcl2.cpp`，`xcl2.hpp`を選択してFinish
...
- コンパイルオプションに`--kernel_frequncy "0:150|1:300"`を追加．
  ...
  - Tool Settingsタブを開く
  - SDx XOCC Kernel Compiler → Miscellaneos で `--kernel_frequency "0:150|1:300"` を追加
...
```
- `*_afi_id.txt`を開いてFpgaImageIdを確認(たとえば，`cat *_afi_id.txt`を実行)
- AFIイメージの作成をまつ(↓コマンドを実行してStateがavailableになるのを待つ)
```
aws ec2 describe-fpga-images --fpga-image-ids 確認したFpgaImageId
```
- `axonerve_kvs.exe` と `binary_container_1.awsxclbin` を `scp` などで手元にダウンロードする
```
scp -i pemファイル centos@AWSインスタンスのIP:axonerve_kvs.exe .
scp -i pemファイル centos@AWSインスタンスのIP:binary_container_1.awsxclbin .
```
...
```
- `axonerve_kvs.exe` と `binary_container_1.awsxclbin` をアップロードする
```
scp -i pemファイル axonerve_kvs.exe centos@AWSインスタンスのIP:
scp -i pemファイル binary_container_1.awsxclbin centos@AWSインスタンスのIP:
```
- 実行する
```
sudo -s
